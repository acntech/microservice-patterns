...
The services exposes REST endpoints. An order is placed in the ordering service. Ordering calls warehouse to reserve the order.
...
##### Tracing
Each call to the ordering service creates a new trace-id which is propagated to remote services. This makes it possible to trace a call between services.
