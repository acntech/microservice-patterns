...
public class ShipmentService {
    ...
    @Transactional
    public void createShipment(@NotNull @Valid final CreateShipmentDto createShipment) {
        LOGGER.debug("Creating shipment for CreateShipmentDto with order-id {}", createShipment.getOrderId());
        final var shipment = conversionService.convert(createShipment, ShipmentEntity.class);
        Assert.notNull(shipment, "Failed to convert CreateShipmentDto to ShipmentEntity");
        final var savedShipmentEntity = shipmentRepository.save(shipment);
        LOGGER.info("Created shipment for order with order-id {}", savedShipmentEntity.getOrderId());
    }
