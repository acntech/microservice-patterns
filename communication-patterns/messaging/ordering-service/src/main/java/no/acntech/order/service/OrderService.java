...
public class OrderService {
    ...
    public OrderDto updateOrderItem(@NotNull final UUID itemId,
        ...
        final var updatedOrderItemEntity = orderItemRepository.save(orderItemEntity);

        final var orderEntity = orderRepository.findByOrderId(orderId)
                .orElseThrow(() -> new OrderNotFoundException(orderId));

        LOGGER.debug("Updated order item status to {} for order-item-id {}", updatedOrderItemEntity.getStatus(), updatedOrderItemEntity.getItemId());
        return convert(orderEntity);
    }

    @Transactional
    public void updateOrderItemReservation(@NotNull @Valid final UpdateOrderItemReservationDto updateOrderItemReservationDto) {
        final var orderItemEntity = orderItemRepository
                .findByParent_OrderIdAndProductId(updateOrderItemReservationDto.getOrderId(), updateOrderItemReservationDto.getProductId())
                .orElseThrow(() -> new OrderItemNotFoundException(updateOrderItemReservationDto.getOrderId(), updateOrderItemReservationDto.getProductId()));

        orderItemEntity.setStatus(updateOrderItemReservationDto.getStatus());
    ...
    private OrderItemDto convert(final OrderItemEntity orderItemEntity) {
        final var orderItemDto = conversionService.convert(orderItemEntity, OrderItemDto.class);
        Assert.notNull(orderItemDto, "Failed to convert OrderItemEntity to OrderItemDto");
        return orderItemDto;
    }
}
