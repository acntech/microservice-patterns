...
public class OrderService {
    ...
    public List<OrderItemDto> findOrderItems(@NotNull final OrderItemQuery query) {
        if (query.getOrderId() != null) {
            return itemRepository.findAllByParentOrderId(query.getOrderId(), SORT_BY_ID).stream()
                    .map(this::convert)
                    .toList();
        } else {
            return itemRepository.findAll(SORT_BY_ID).stream()
                    .map(this::convert)
                    .toList();
        }
    }
    ...
    public OrderDto updateOrderItem(@NotNull final UUID itemId,
        ...
        orderItemEntity.setQuantity(updateOrderItemDto.getQuantity().getValue());
