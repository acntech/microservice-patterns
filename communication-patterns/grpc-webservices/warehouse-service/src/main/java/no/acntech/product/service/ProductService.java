...
public class ProductService {
    ...
    public ProductDto getProduct(@NotNull final UUID productId) {
        LOGGER.debug("Get product {}", productId);
        return productRepository.findByProductId(productId)
                .map(this::convert)
                .orElseThrow(() -> new ProductNotFoundException(productId));
    }

    public List<ProductDto> findProducts(@NotNull @Valid final ProductQuery query) {
        if (query.getName() != null) {
            LOGGER.debug("Find products by name {}", query.getName());
            return productRepository.findAllByName(query.getName())
                    .stream()
                    .map(this::convert)
    ...
    public ProductDto createProduct(@NotNull @Valid final CreateProductDto createProductDto) {
        LOGGER.debug("Create product with code {}", createProductDto.getCode().getValue());
